...
public class VoitureFeignClientFallback implements VoitureFeignClient {
    ...
    public VoitureDTO getVoitureById(Long id) {
        System.err.println("Fallback: Le service-voiture est inaccessible - getVoitureById(" + id + ")");
        return null;
    }

    @Override
    public List<VoitureDTO> getVoituresByClientId(Long idClient) {
        System.err.println("Fallback: Le service-voiture est inaccessible - getVoituresByClientId(" + idClient + ")");
        return Collections.emptyList();
    }
}
